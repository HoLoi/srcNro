...
public class TrungUyXanhLoBdkb extends Boss {
    ...
    public void reward(Player plKill) {
        if (levell == 110) {
            int a = 0;
            for (int i = 0; i < 40; i++) {
                ItemMap it = new ItemMap(this.zone, 861, 1000, this.location.x, this.zone.map.yPhysicInTop(this.location.x,
            ...
            int a = 0;
            for (int i = 0; i < 40; i++) {
                ItemMap it = new ItemMap(this.zone, 861, 1500, this.location.x, this.zone.map.yPhysicInTop(this.location.x,
                        this.location.y - 24), -1);
                Service.getInstance().dropItemMap(this.zone, it);
                a += 10;
            }
            ...
            int a = 0;
            for (int i = 0; i < 40; i++) {
                ItemMap it = new ItemMap(this.zone, 861, 2500, this.location.x, this.zone.map.yPhysicInTop(this.location.x,
                        this.location.y - 24), -1);
                Service.getInstance().dropItemMap(this.zone, it);
                a += 10;
            }
